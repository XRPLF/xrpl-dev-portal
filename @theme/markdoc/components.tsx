    ...
    <div className="interactive-block" id={'interactive-' + stepId} key={pathname}>
      <div className="interactive-block-inner">
        <div className="breadcrumbs-wrap">
          <ul className="breadcrumb tutorial-step-crumbs" id={'bc-ul-' + stepId} data-steplabel={props.label} data-stepid={stepId}>
...
export function RepoLink(props: { children: React.ReactNode; path: string; github_fork: string; github_branch: string }) {
  ...
  return <Link to={href}>{dynamicReact(props.children, React, {})}</Link>
}

export function CodePageName(props: { name: string }) {
  return <code>{props.name}</code>
}

type TryItServer = 's1' | 's2' | 'xrplcluster' | 'testnet' | 'devnet' | 'testnet-clio' | 'devnet-clio'

export function TryIt(props: { method: string; server?: TryItServer }) {
  ...
  return (
    <Link style={{ marginBottom: '1rem', textDecoration: 'none' }} className="btn btn-primary btn-arrow" to={to_path} target="_blank" role="button">
      {translate('component.tryit', 'Try it!')}
    </Link>
  )
}

export function TxExample(props: { txid: string; server?: TryItServer }) {
  const { useTranslate } = useThemeHooks()
  const { translate } = useTranslate()
  let use_server = ''
  if (props.server == 's1') {
...
export function NotEnabled() {
  ...
  const { translate } = useTranslate()
  return (
    <span className="status not_enabled" title={translate('This feature is not currently enabled on the production XRP Ledger.')}>
