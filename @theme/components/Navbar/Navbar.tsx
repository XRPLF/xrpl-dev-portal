...
export function NavDropdown(props) {
  ...
  const dropdownGroups = items.map((item, index) => {
    if (item.items) {
      ...
      return (
        <Link key={index} className="dropdown-item dropdown-hero" id={hero_id} to={hero_href}>
...
export function NavWrapper(props) {
  return (
    <nav className="top-nav navbar navbar-expand-lg navbar-dark fixed-top" style={props.belowAlertBanner ? { marginTop: '52px' } : {}}>
      {props.children}
    </nav>
  )
}
...
export function GetStartedButton() {
  ...
  return (
    <Link className="btn btn-primary" to={'/docs/tutorials'} style={{ height: '38px', paddingTop: '11px' }}>
    ...
    return (
      <div className="nav-item" id="topnav-theme">
        <form className="form-inline">
          ...
          >
            <input type="checkbox" className="custom-control-input" id="css-toggle-btn" onClick={this.user_choose_theme} />
