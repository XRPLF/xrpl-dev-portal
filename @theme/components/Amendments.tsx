  ...
  return (
    <div className="md-table-wrapper">
      <table className="md">
        <thead>
          <tr>
            <th align="left" data-label="Name">
              {translate('amendment.table.name', 'Name')}
            </th>
            <th align="left" data-label="Introduced">
              {translate('amendment.table.introduced', 'Introduced')}
            </th>
            <th align="left" data-label="Status">
              {translate('amendment.table.status', 'Status')}
            </th>
          </tr>
        </thead>
        <tbody>
...
export function AmendmentDisclaimer(props: { name: string; compact: boolean; mode: string }) {
  ...
  const link = () => (
    <Link to={`/resources/known-amendments#${props.name.toLowerCase()}`}>
      {props.name}
      {props.compact ? '' : ' amendment'}
    </Link>
  )
  ...
  if (loading) {
    return (
      <p>
        <em>
          {translate('component.amendment-status.requires.1', 'Requires the ')}
          {link()}
  ...
  if (props.compact) {
    return (
      <>
        {link()} <AmendmentBadge amendment={amendmentStatus} />
      </>
    )
  }
      ...
      <p>
        <em>
          ...
          {amendmentStatus.date ? (
            <>
              {translate('component.amendment-status.updated.1', 'Updated by the ')}
              {link()}
              {translate('component.amendment-status.updated.2', '.')} <AmendmentBadge amendment={amendmentStatus} />
            </>
          ) : (
            <>
              {translate('component.amendment-status.updates.1', 'The ')}
              {link()}
              {translate('component.amendment-status.updates.2', ' updates this.')} <AmendmentBadge amendment={amendmentStatus} />
            </>
          )}
          )
        </em>
  ...
  return (
    <p>
      <em>
        ...
        {amendmentStatus.date ? (
          <>
            {translate('component.amendment-status.added.1', 'Added by the ')}
            {link()}
            {translate('component.amendment-status.added.2', '.')} <AmendmentBadge amendment={amendmentStatus} />
          </>
        ) : (
          <>
            {translate('component.amendment-status.requires.1', 'Requires the ')}
            {link()}
            {translate('component.amendment-status.requires.2', '.')} <AmendmentBadge amendment={amendmentStatus} />
          </>
        )}
        )
      </em>
...
function shieldsIoEscape(s: string) {
  return s.trim().replace(/-/g, '--').replace(/_/g, '__').replace(/%/g, '%25')
}

export function Badge(props: { children: React.ReactNode; color: string; href: string }) {
  ...
  if (props.href) {
    ...
    return <img src={badge_url} alt={childstrings} className="shield" />
  }
}
