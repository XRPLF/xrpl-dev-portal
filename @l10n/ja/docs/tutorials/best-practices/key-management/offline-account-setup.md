...
- オンラインマシンとして使用する別のコンピュータを用意していること。このマシンは`xrpld`を実行する必要はありませんが、XRP Ledgerネットワークに接続し、共有レジャーの状態についての正確な情報を受信できる必要があります。例えば、[公開サーバへのWebSocket接続](../../get-started/get-started-http-websocket-apis.md)を使用できます。
...
- パッケージ（`.deb`または`.rpm`。使用するLinuxディストリビューションによって異なる）ファイルから[`xrpld`をインストール](../../../infrastructure/installation/index.md)し、[スタンドアロンモードで実行します](../../../concepts/networks-and-servers/xrpld-server-modes.md)。
...
**オフラインマシン**で、アカウントで使用する[暗号鍵](../../../concepts/accounts/cryptographic-keys.md)のペアを生成します。鍵は、単純なパスフレーズやエントロピーが十分でないその他のソースから生成するのではなく、安全なランダム手続きで生成してください。（例えば、`xrpld`の[wallet_proposeメソッド][]を使用することができます。）

{% tabs %}

{% tab label="xrpldコマンドライン" %}
```sh
