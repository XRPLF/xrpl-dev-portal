...
- C++: `xrpld`コードベース:
    ...
    - {% source-link name="汎用キー & Ed25519鍵導出" path="src/libxrpl/protocol/SecretKey.cpp" /%}
    - {% source-link name="secp256k1鍵導出" path="src/libxrpl/protocol/SecretKey.cpp" /%}
...
1. 次のように、シード値から「ルートキーペア」を計算します。
    ...
    3. 結果が有効なsecp256k1秘密鍵でない場合は、ルートシーケンスを1増やして最初からやり直します。{% source-link path="src/libxrpl/protocol/SecretKey.cpp" /%}
