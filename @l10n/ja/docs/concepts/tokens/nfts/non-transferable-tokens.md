...
XRP Ledger内のNFTで**Transferable**フラグが無効になっているものは、譲渡不可トークンです。このフラグは、NFTをミントするために利用する[NFTokenMintトランザクション][]にて設定されます。NFTがミントされると、譲渡可能かどうかは[NFTokenオブジェクト](/ja/docs/references/protocol/data-types/nftoken/)の`flagTransferable`フラグで記録されます。
