...
---
# TicketCreate
{% source-link path="src/libxrpl/tx/transactors/system/TicketCreate.cpp" /%}
...
{% admonition type="success" name="ヒント" %}このトランザクションは、送信アカウントの[シーケンス番号][]を1 _+_ 作成するチケットの数(`TicketCount`)だけ増加させます。この取引は、アカウントのシーケンス番号を1より多く増加させる唯一の取引です。{% /admonition %}
