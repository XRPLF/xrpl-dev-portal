...
seo:
    description: xrpldサーバのトランザクション結果の解釈の仕方について説明します。
labels:
  - トランザクション送信
---
# トランザクションの結果
...
| ローカルエラー           | [tel](tel-codes.md)   | 負荷が高いなど、ローカルの状態が原因となって、`xrpld`サーバでエラーが発生しました。サーバまたは時間を変えて再送信すると、別のレスポンスを得られる可能性があります。 |
...
[submitメソッド][]から返されるレスポンスには、トランザクションのローカル処理中に発生した事項を示す、`xrpld`サーバからの暫定的な結果が含まれています。
