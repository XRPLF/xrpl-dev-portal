...
| `Field`                                | 型            | 説明          |
|:---------------------------------------|:--------------|:---------------------|
| `complete_ledgers`                     | 文字列         | ローカル`xrpld`がデータベース内に有するレジャーのバージョンのシーケンス番号の範囲を示す表現。例えば、`24900901-24900984,24901116-24901158`のように、互いに素なシーケンスの場合があります。サーバに完全なレジャーがない場合（例えば、ネットワークとの同期を始めたばかりの場合）、文字列`empty`になります。 |
...
| `rpc.*.forwarded`                      | 数値           | 起動してからClioサーバが`xrpld`P2Pサーバに転送したAPI呼び出しの数。 |
...
| `validation_quorum`                    | 数値           | _(省略される場合があります)_ レジャーバージョンを検証するために必要な、信頼できる検証の最小数。状況によっては、サーバがより多くの検証をリクエストすることもあります。この値は`xrpld`から取得します。何らかの理由でClioサーバが`xrpld`に接続できない場合、このフィールドはレスポンスから省略されることがあります。 |
...
| `etl`                                  | オブジェクト    | Clioサーバが接続している`xrpld`ソース(ETLソース)。クライアントが`localhost`経由で Clioサーバに接続した場合のみ表示されます。 |
| `etl.etl_sources`                      | オブジェクト配列 | Clioサーバが接続され、データを抽出する`xrpld`ソース(ETLソース)を表示します。 |
| `etl.etl_sources.validated_range`      | 文字列         | P2Pの`xrpld`サーバから取得した有効なレジャーの範囲。 |
| `etl.etl_sources.is_connected`         | 真偽値         | ClioがWebSocket経由でこのソースに接続されていればTrue、そうでなければFalse。ここでfalseが指定された場合は、ネットワークの問題や `xrpld`が実行されていないことなどが考えられます。 |
| `etl.etl_sources.ip`                   | 数値           | `xrpld`サーバのIP。 |
| `etl.etl_sources.ws_port`              | 数値           | `xrpld`サーバのWebSocketポート。 |
| `etl.etl_sources.grpc_port`            | 数値           | Clioサーバが接続しているP2P`xrpld`サーバのgRPC接続ポート。 |
| `etl.etl_sources.last_msg_age_seconds` | 数値           | Clioが最後に`xrpld`から何かを取得してからの経過秒数の合計。これは8より大きくなってはいけません。 |
...
| `validated`                            | 真偽値         | Trueの場合、レスポンスがコンセンサスによって検証されたレジャーバージョンを使用していることを示します。Clioでは、検証済みのレジャーデータを保存して返すため、これは常にtrueです。リクエストが`xrpld`に転送され、サーバが現在のデータを返した場合、値がないかfalseの場合は、このレジャーのデータが確定でないことを示します。 |
