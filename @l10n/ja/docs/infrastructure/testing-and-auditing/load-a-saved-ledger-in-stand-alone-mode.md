...
以前にディスクに保存していた[履歴レジャーバージョン](../../concepts/ledgers/index.md)を使用して、`xrpld`サーバを[スタンドアロンモード](../../concepts/networks-and-servers/xrpld-server-modes.md)で起動できます。例えば、以前に`xrpld`サーバをXRP Ledgerのピアツーピアネットワーク（[本番Mainnet、Testnet、Devnetなど](../../concepts/networks-and-servers/parallel-networks.md)）と同期していた場合は、過去にサーバで使用できていた任意のレジャーバージョンを読み込むことができます。
...
## 1. `xrpld`を通常の方法で起動します。

既存のレジャーを読み込むには、最初にネットワークから当該のレジャーを取得する必要があります。`xrpld`をオンラインモードで通常の方法で起動します。

```
xrpld --conf=/path/to/xrpld.cfg
```

## 2. `xrpld`が同期されるまで待ちます。
...
特定の履歴レジャーバージョンを読み込むには、[ledger_requestメソッド][]を実行して`xrpld`にそのレジャーバージョンを取得させます。`xrpld`にまだレジャーバージョンがない場合は、レジャーの取得が完了するまで`ledger_request`コマンドを複数回実行する必要があります。
...
```
xrpld stop --conf=/path/to/xrpld.cfg
```

## 5. スタンドアロンモードで`xrpld`を起動します。

最新のレジャーバージョンを読み込むには、`-a`オプションと`--load`オプションを指定してサーバを起動します。

```
xrpld -a --load --conf=/path/to/xrpld.cfg
```

特定の履歴レジャーを読み込むには、`--load`パラメーターと`--ledger`パラメーターを使用し、読み込むレジャーバージョンのレジャーインデックスまたは識別用ハッシュを指定してサーバを起動します。

```
xrpld -a --load --ledger 19860944 --conf=/path/to/xrpld.cfg
```

スタンドアロンモードで`xrpld`を起動するときに使用可能なオプションについての詳細は、[コマンドラインの使用: スタンドアロンモードのオプション ](../commandline-usage.md#スタンドアロンモードのオプション)をご覧ください。
...
```
xrpld ledger_accept --conf=/path/to/xrpld.cfg
```
...
- **コンセプト:**
    - [`xrpld`サーバ](../../concepts/networks-and-servers/index.md)
      - [`xrpld`サーバのモード](../../concepts/networks-and-servers/xrpld-server-modes.md)
