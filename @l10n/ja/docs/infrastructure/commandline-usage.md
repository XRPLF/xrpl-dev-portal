...
| `--ledgerfile {FILE}` | 指定された`{FILE}`からレジャーバージョンを読み込みます（このファイルには完全なレジャーがJSONフォーマットで格納されている必要があります）。このようなファイルの例については、付属の{% repo-link repo="devportal" path="_api-examples/rippled-cli/ledger-file.json" %}`ledger-file.json`{% /repo-link %}をご覧ください。 |
