...
| コアサーバ, XRP Ledgerのコアサーバ | `xrpld` | `xrpld`という名前は近い将来廃止される可能性が高いので、より一般的な名前で呼ぶことを推奨します。必要なときは、`xrpld` をすべて小文字で、コードフォントで呼んでください。(発音は "リップルディー" で、"d" は UNIX の伝統に従って"daemon"を意味します)。
...
seo:
  description: xrpldはXRP Ledgerを管理するコアとなるピアツーピアサーバです。このセクションでは、xrpldサーバの基本的な機能の背景にある「要素」と「その理由」を学ぶのに役立つ概念について説明します。
---
```
...
| `server`   | いいえ    | リクエストに使用する特定のサーバ。Clio または `xrpld` サーバに固有のメソッドを使用する場合、または、例で使用するデータやAmendmentが特定のテストネットワーク上のみにある場合、サーバを指定する必要があります。 |
...
| `xrplcluster`  | 軽量プロキシを前面に配置した`xrpld`サーバーが提供する`xrplcluster.com`公開サーバークラスタ |
