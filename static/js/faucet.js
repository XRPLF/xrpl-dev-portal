async function wait_for_seq(network_url, address) {
  ...
  $('#balance').html('<h3>Balance</h3> ' + (Number(response.result.account_data.Balance) * 0.000001).toLocaleString('en') + ' XRP')
  api.disconnect()
}
...
function TestNetCredentials(url, altnet_name, ws_url) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      balance
        .hide()
        .html('<h3>Balance</h3> ' + Number(data.amount).toLocaleString('en') + ' XRP')
        .fadeIn('fast')
