...
onCurrentRouteLoaded(() => {
  ...
  $('#check-for-amm').click(async (event) => {
    ...
    try {
      ...
      if (err.data.error === 'actNotFound') {
        show_log(
          block,
          `<p>✅ No AMM exists yet for the pair
  ...
  $('#look-up-ammcreate-cost').click(async (event) => {
    ...
    try {
      ...
      show_log(
        block,
        `<p>Current AMMCreate transaction cost: ${xrpl.dropsToXrp(amm_fee_drops)} XRP (<span id="ammcreate-cost-drops">${amm_fee_drops}</span> drops)</p>`,
      )
  ...
  $('#create-amm').click(async (event) => {
    ...
    if (ammcreate_result.result.meta.TransactionResult == 'tesSUCCESS') {
      show_log(
        block,
        `<p><a href="${EXPLORER}/transactions/${ammcreate_result.result.hash}">AMM created</a>:</p>
  ...
  $('#check-amm-info').click(async (event) => {
    ...
    try {
      ...
      show_log(
        block,
        `<p>The AMM account <strong>${lp_token.issuer}</strong> has <strong>${lp_token.value}</strong> total
      ...
      show_log(
        block,
        `<p>In its pool, the AMM holds <strong>${amount.value} ${amount.currency}.${amount.issuer}</strong>
