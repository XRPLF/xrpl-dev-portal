...
onCurrentRouteLoaded(() => {
  ...
  $('#prepare-and-sign').click(async function (event) {
    ...
    block.find('.output-area').append(`<p>Transaction hash: <code id="tx_id">${hash}</code></p>`)
    block.find('.output-area').append(`<p>Signed blob:</p><pre class="tx-blob"><code id="tx_blob">${tx_blob}</code></pre>`)
  ...
  $('#check-tickets').click(async function (event) {
    ...
    block.find('.output-area').html(`<pre><code>${pretty_print(response)}</code></pre>`)
  ...
  $('#prepare-ticketed-tx').click(async function (event) {
    ...
    block.find('.output-area').append(`<p>Transaction hash: <code id="tx_id_t">${hash}</code></p>`)
