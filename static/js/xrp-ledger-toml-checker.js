...
const ACCOUNT_FIELDS = ['address', 'network', 'desc']
const VALIDATOR_FIELDS = ['public_key', 'network', 'owner_country', 'server_country', 'unl']
const PRINCIPAL_FIELDS = ['name', 'email']
const SERVER_FIELDS = ['json_rpc', 'ws', 'peer', 'network']
const CURRENCY_FIELDS = ['code', 'display_decimals', 'issuer', 'network', 'symbol']
        ...
        if (data.result.account_data.Domain) {
          try {
            ...
            checkingLog
              .resolve('ERROR')
              .addClass(CLASS_BAD)
              .after('<p>Error decoding domain field: ' + data.result.account_data.Domain + '</p>')
          }
        } else {
...
async function parseXRPLTomlWallet(data) {
  ...
  async function listEntriesWallet(name, list, fields) {
    ...
    if (found) {
      ...
      makeLogEntryWallet('Account has been found in TOML file and validated.')
        .resolve('DOMAIN VALIDATED <i class="fa fa-check-circle"></i>')
        .addClass(CLASS_GOOD)
    } else {
      let entry_def = $('<ul>').appendTo(list_ol)
