...
function complete_step_by_id(step_id) {
  ...
  const next_ui = $(`#interactive-${step_id}`).nextAll('.interactive-block').eq(0).find('.previous-steps-required')
  next_ui.prop('title', '')
    ...
    api.on('ledgerClosed', async (ledger) => {
      ...
      try {
        ...
        if (e.data.error == 'txnNotFound' && e.data.searched_all) {
          status_box.html(`<th>${tl('Final Result:')}</th><td>${tl('Failed to achieve consensus (final)')}</td>`)
        } else {
          status_box.html(`<th>${tl('Final Result:')}</th><td>${tl('Unknown')}</td>`)
        }
      }
...
async function activate_wait_step(step_name, prelim) {
  ...
  if (wait_step.find('.waiting-for-tx').text() == tx_id) {
    ...
    wait_step.find('.earliest-ledger-version').text(prelim.result.validated_ledger_index)
  }
...
async function generic_full_send(event, tx_json, wallet) {
  ...
  block.find('.output-area').append(`<p>${tl('Transaction hash:')} <code id="tx_id">${hash}</code></p>`)
