...
import { WS_URL } from './util/consts'
...
async function main() {
  ...
  const { account_lines_result: account_lines_result, ammInfo: ammInfo } = await confirmAmm(client, wallet, amm_info_request)
  ...
  // confirm AMM again
  const { account_lines_result: account_lines_result2 } = await confirmAmm(client, wallet, amm_info_request)
  ...
  // Confirm that AMM exists --------------------------------------------------
  const { ammInfo: ammInfo2 } = await confirmAmm(client, wallet, amm_info_request2)
