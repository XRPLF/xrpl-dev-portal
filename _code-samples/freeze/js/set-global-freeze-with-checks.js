...
async function main() {
  ...
  const response = await client.request({ command: 'account_info', account: wallet.address })
  const settings = response.result
  const lsfGlobalFreeze = 0x00400000

  console.log(settings)
  console.log('Got settings for address', wallet.address)
  console.log('Global Freeze enabled?', (settings.account_data.Flags & lsfGlobalFreeze) === lsfGlobalFreeze)
  ...
  // Checking the status of the global freeze -----------------------------------
  const response2 = await client.request({ command: 'account_info', account: wallet.address })
  const settings2 = response2.result

  console.log(settings2)
  console.log('Got settings for address', wallet.address)
  console.log('Global Freeze enabled?', (settings2.account_data.Flags & lsfGlobalFreeze) === lsfGlobalFreeze)
