 ...
 *  binary - Credential type (binary) as hexadecimal.
...
async function verifyCredential(client, issuer, subject, credentialType, binary=false) {
  ...
  if (!credentialType) {
    throw new Error("Provide a non-empty credential_type or credential_type_hex");
  }
  ...
  if (credentialTypeHex.length % 2 !== 0 || !CREDENTIAL_REGEX.test(credentialTypeHex)) {
    ...
    throw new Error("credential_type_hex must be 1-64 bytes as hexadecimal.");
  }
  ...
  if (credential.Expiration) {
    ...
    const closeTime = rippleTimeToISOTime(
