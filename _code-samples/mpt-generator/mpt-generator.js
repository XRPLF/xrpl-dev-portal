function generateCode() {
  let v_flags = 0
  if (clawbackSlider.checked) {
    v_flags += 64
  }
  if (lockSlider.checked) {
    v_flags += 2
  }
  if (authTokensSlider.checked) {
    v_flags += 4
  }
  if (txrSlider.checked) {
    v_flags += 32
  }
  if (tradeSlider.checked) {
    v_flags += 16
  }
  if (escrowSlider.checked) {
    v_flags += 8
  }
  ...
  let v_codeBlock =
    ...
    accountField.value +
    '",\n  "AssetScale": ' +
    parseInt(assetScaleField.value) +
    ', \n  "MaximumAmount": "' +
    maximumAmountField.value +
    '",\n  "TransferFee": ' +
    transferFeeField.value +
    ',\n  "Flags": ' +
    v_flags +
    ',\n  "MPTokenMetadata": "' +
    mptHexString +
