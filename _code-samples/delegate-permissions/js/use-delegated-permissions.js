...
readline.question(`Delegator's address? `, async function (delegator_address) {
  readline.question(`Delegate's seed? `, async function (secret) {
    ...
    for (delegate_entry of response.result.account_objects) {
      if (delegate_entry.Account == delegator_address && delegate_entry.Authorize == delegate_wallet.address) {
        found_match = true
    ...
    if (!found_match) {
      console.warn('Delegate appears not to have any permissions granted' + ' by the delegator.')
      console.warn('Make sure you ran delegate-permissions.js and input the' + ' correct delegate/delegator values to this script.')
      return
    }
