    ...
    <div className="landing dev-blog">
      <div className="justify-content-center align-items-lg-center">
        <div className="position-relative d-none-sm">
          <img alt="background purple waves" src={require('../static/img/backgrounds/home-purple.svg')} id="blog-purple" />
        </div>
        ...
        <section className="container-new">
          <div className="row justify-content-center align-items-lg-center">
            ...
            <div className="image-container">
              <img alt="blog hero" src={require('../static/img/blog/blog-hero.svg')} className="w-100" />
            </div>
            {/* Text */}
            <div className="col">
              <div className="text-bg">
                <h4 className="mb-3 eyebrow text-uppercase font-weight-light">
                  <span className="hero-post-date pb-2">{moment(heroPost.date).format(translate('blog.banner.date.part1', 'MMM'))}</span>
                  {translate('blog.banner.date.part2', ' ')}
                  {moment(heroPost.date).format(translate('blog.banner.date.part3', 'DD YYYY'))}
                </h4>
                <div className="pb-8">
                  <div className={`d-inline-block mb-2 label blog-category-${heroPost.category_id}`}>{translate(`${heroPost.category}`)}</div>
                </div>
                <h4 className="mb-8 h2-sm font-weight-bold">
                  <Link to={`/blog/${heroPost.link}`}>{translate(`${heroPost.title}`)}</Link>
                </h4>
                <p className="mb-4">{translate(`${heroPost.description}`)}</p>
                <div className="d-lg-block">
                  <Link className="btn btn-primary btn-arrow" to={`/blog/${heroPost.link}`}>
                    {translate('Read More')}
                  </Link>
                </div>
              </div>
          ...
          <div className="row w-100 mx-auto px-2">
            <div className="col-lg-4 m-0 p-0 mt-2">
                  ...
                  {Object.keys(categories).map((item) => (
                    <div key={item} className="category-checkbox pb-2">
                      <input
                        ...
                        defaultChecked
                      />
                      <label className="font-weight-bold" htmlFor={`input_desktop_${item}`}>
              ...
              <div className="col d-flex flex-column p-0 d-lg-none mb-4">
                ...
                <div className="dropdown">
                  ...
                  {open && (
                    <div className="dropdown-content d-flex flex-column mt-2" aria-labelledby="blog-dropdown-button">
                      {Object.keys(categories).map((item, i) => (
                        <div key={item + i} className="category-checkbox pl-2 pb-2">
                          <input
                            ...
                            defaultChecked
                          />
                          <label className="font-weight-bold" htmlFor={`input_mobile_${item}`}>
                            {translate(categories[item])}
                          </label>
                        </div>
                      ))}
            ...
            <div className="row col row-cols-lg-2 m-0 p-0">
              {filteredCards.map((card, i) => (
                <div key={card.title + i} className={`${card.category_id} pb-5 px-lg-4`} id={card.title + i}>
                  <div className="mb-4 category-list">
                    <img alt="" className="mb-4" />
                    <div className={`d-block label blog-category-${card.category_id}`}>{translate(card.category)}</div>
                  </div>
                  <div>
                    ...
                    <h5 className="mb-2-sm h3-sm">
                      <Link to={`/blog/${card.link}`}>{translate(card.title)}</Link>
                    </h5>
                  </div>
                  <div className="d-lg-block">
                  ...
                  <div className="d-lg-block">
                    <Link className="btn btn-primary btn-arrow" to={`/blog/${card.link}`}>
                      {translate('Read More')}
                    </Link>
                  </div>
                </div>
