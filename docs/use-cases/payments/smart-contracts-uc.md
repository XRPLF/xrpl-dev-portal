...
<img class="uc-smart-contract-condition-diagram" src="/img/uc-smart-contract-condition.svg" alt="A smart contract holding a Condition: if the condition is met the action executes, and if it is not met nothing happens.">
...
<img class="uc-escrow-flow-diagram" src="/img/uc-conditional-escrow-flow.svg" alt="The sender submits EscrowCreate to place funds in a held escrow locked by a Condition; the recipient releases them with an EscrowFinish transaction carrying the matching Fulfillment.">
...
<img class="uc-escrow-oracle-diagram" src="/img/uc-escrow-oracle.svg" alt="An oracle acting as a neutral third party verifies real-world data, giving the Condition to the escrow creator at setup and the Fulfillment to the recipient after verification.">
