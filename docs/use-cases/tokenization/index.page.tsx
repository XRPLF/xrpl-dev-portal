...
export default function Tokenization() {
  ...
  return (
    <div className="landing page-tokenization">
        ...
        <div className="card-grid card-grid-2xN">
          ...
          <div className="col">
            <div className="d-flex flex-column-reverse">
              ...
              <h2 className="h4 h2-sm mb-10">{translate('Work with a variety of tokens supported by the XRP Ledger.')}</h2>
              <h6 className="eyebrow mb-3 text-large">{translate('Tokenization')}</h6>
            </div>
            <p className="mb-10">
              {translate(
                'Explore the possibilities of tokenization on the XRP Ledger, revolutionizing ownership, transactions, and value exchange with unparalleled efficiency and security.',
            ...
            <div className="d-flex">
              <Link className="btn btn-primary d-inline-flex align-items-center" to="/docs">
      ...
      <section className="container-new py-20">
        ...
        <div className="tokenization-stats">
          {stats.map((statElement) => (
            <div className="stat-container align-items-center" key={statElement.id}>
      ...
      <ProjectCards title="Featured real world projects" projects={projects} showCarousel={true} />
      ...
      <section className="container-new py-20">
        ...
        <div className="related-articles card-grid card-grid-3xN mb-16">
          {articles.map((article, index) => (
            <div className="article-card-container" key={`article-${index}`}>
              ...
              <a className="article-card col p-8 float-up-on-hover" target="_blank" href={article.url}>
                <div className="time h4 normal mb-8">{translate(article.time)}</div>
                <div className="h5 mb-4">{translate(article.title)}</div>
                <div className="btn btn-primary btn-arrow-out">{translate('Read More')}</div>
              </a>
            </div>
          ))}
        </div>
        <div className="more-related">
          <div className="d-flex flex-column-reverse">
            <h4 className="eyebrow mb-16">{translate('More related articles')}</h4>
          </div>
          <div className="video-external-link">
            <a href="https://coincodecap.com/ripple-xrp">
              {translate('Ripple (XRP): A Dive into its Working, Tokenomics, Price Factor and SEC Lawsuit')}
            </a>
          </div>
