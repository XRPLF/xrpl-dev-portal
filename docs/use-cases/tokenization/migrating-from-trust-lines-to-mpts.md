...
| Mutable properties | ✅ Supported.<br><br>Account-wide. Some settings (for example, **No Freeze**) can't be reversed. | ✅ Supported: {% amendment-disclaimer name="DynamicMPT" compact=true /%}<br><br>Per-issuance. Metadata and transfer fee are mutable by default unless made permanently immutable. Flags are mutable by default too, but can only be enabled, never disabled, unless made permanently immutable. |
