...
- **Signing location:** Always local and in-process. The skill never sends a seed to a remote `rippled` sign API, and never sends it across a network boundary the developer does not control.
