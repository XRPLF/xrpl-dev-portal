...
{% admonition type="info" name="Note" %}The code samples on this page illustrate how a bridge was set up between _Devnet_ and _Sidechain-Devnet_, using a supported [client library](/docs/references/client-libraries.md) to query and submit transactions. This bridge is already created, so the process can't be reproduced on these networks.{% /admonition %}
