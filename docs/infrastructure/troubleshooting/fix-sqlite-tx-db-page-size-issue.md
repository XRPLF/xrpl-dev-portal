...
{% admonition type="success" name="Tip" %}The location of the debug log depends on your `xrpld` server's config file. The {% source-link name="default configuration" path="cfg/xrpld-example.cfg#L1139-L1142" /%} writes the server's debug log to the file `/var/log/xrpld/debug.log`.{% /admonition %}
