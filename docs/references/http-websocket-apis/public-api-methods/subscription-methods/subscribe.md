...
- _Stream: `server`_ - Information about the server status, such as `load_base` (the current load level of the server), `random` (a randomly-generated value), and others, subject to change.
- _Stream: `transactions`_, _Stream: `transactions_proposed`_, _Stream: `validations`_, and _Stream: `consensus`_ - No fields returned.
- _Stream: `ledger`_ - Information about the ledgers on hand and current fee schedule. This includes the same fields as a [ledger stream message](#ledger-stream), except that it omits the `type` and `txn_count` fields.
