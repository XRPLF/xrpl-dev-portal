...
| `state_accounting`                  | Object          | A map of various [server states](../../api-conventions/xrpld-server-states.md) with information about the time the server spends in each. This can be useful for tracking the long-term health of your server's connectivity to the network. The contents of this field are formatted as [State Accounting Objects](#state-accounting-object). || `time`                              | String          | The current time in UTC, according to the server's clock. |
