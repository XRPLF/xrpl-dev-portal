...
| `tecFROZEN`             | A token being deposited is [frozen](../../../../concepts/tokens/fungible-tokens/freezes.md) globally, for the sender, or for the AMM's account. Both pool assets are checked against the AMM's account, not only the assets named in `Amount` and `Amount2`, so a freeze on one pool asset also blocks a single-asset deposit of the other. {% amendment-disclaimer name="fixCleanup3_3_0" mode="updated" /%} |
...
| `tecLOCKED`             | An MPT being deposited is locked globally, for the sender, or for the AMM's account. Both pool assets are checked against the AMM's account, not only the assets named in `Amount` and `Amount2`, so a lock on one pool asset also blocks a single-asset deposit of the other. {% amendment-disclaimer name="fixCleanup3_3_0" mode="updated" /%} |
| `tecNO_AUTH`            | The sender is not authorized to hold one of the pooled assets. Both pool assets are checked, so a lack of authorization for one blocks depositing the other. |
