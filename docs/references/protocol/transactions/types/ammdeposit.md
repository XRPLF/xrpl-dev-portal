...
| `tecAMM_FAILED`         | The conditions on the deposit could not be satisfied. For example, the requested effective price in the `EPrice` field is too low, or the amount is out of representable range. {% amendment-disclaimer name="fixCleanup3_4_0" /%} Without the amendment enabled, this case fails with `tefEXCEPTION` instead. |
