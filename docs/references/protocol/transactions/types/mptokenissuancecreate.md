...
| `MaximumAmount`   | string              | UInt64            | (Optional) The maximum asset amount of this token that should ever be issued, as a base 10 encoded string. For issuances that do not have a maximum limit, set this value to 9,223,372,036,854,775,807 (2^63-1). _The default value may increase in the future, so this field should always be specified if an upper limit is required for the token._ |
