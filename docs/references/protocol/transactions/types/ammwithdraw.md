...
| `tecAMM_FAILED`         | The conditions on the withdrawal could not be satisfied; for example, the requested effective price in the `EPrice` field is too low, or the amount is out of representable range. {% amendment-disclaimer name="fixCleanup3_4_0" mode="updated" /%} Without the amendment enabled, this case fails with `tefEXCEPTION` instead. |
...
| `tecFROZEN`             | A token being withdrawn is [frozen](../../../../concepts/tokens/fungible-tokens/freezes.md) globally for the AMM's pseudo-account, or [deep frozen](../../../../concepts/tokens/fungible-tokens/freezes.md#deep-freeze) for the sender. A regular freeze on the sender does not cause this error. {% amendment-disclaimer name="fixCleanup3_3_0" mode="updated" /%} |
