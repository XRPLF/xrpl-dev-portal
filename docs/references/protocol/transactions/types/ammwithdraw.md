...
| `tecAMM_FAILED`         | The conditions on the withdrawal could not be satisfied; for example, the requested effective price in the `EPrice` field is too low, or the amount is out of representable range. {% amendment-disclaimer name="fixCleanup3_4_0" /%} Without the amendment enabled, this case fails with `tefEXCEPTION` instead. |
