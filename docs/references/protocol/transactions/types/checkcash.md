...
| `temBAD_CURRENCY` | The `Amount` or `DeliverMin` specifies an invalid currency. |
| `temDISABLED` | `Amount` or `DeliverMin` specifies an MPT but the [MPTokensV2 amendment][] is not enabled. |
| `temMALFORMED` | The transaction specifies both `Amount` and `DeliverMin`, or omits both, or the `Amount` or `DeliverMin` does not match the currency (and issuer, if not XRP) of the Check. This result also occurs if the `CheckID` is an all-zero value; previously, an all-zero `CheckID` resulted in `tecNO_ENTRY`. {% amendment-disclaimer name="fixCleanup3_3_0" mode="updated" compact=true /%} |
