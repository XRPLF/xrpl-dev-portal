...
| `terNO_DELEGATE_PERMISSION` | The `Delegate` isn't authorized to send the transaction or granular flag/field on behalf of the `Account`. {% amendment-disclaimer name="PermissionDelegationV1_1" /%} |
| `terNO_PERMISSION` | The [fee- or reserve-sponsored transaction](../../../../concepts/accounts/sponsored-fees-and-reserves.md) doesn't have permission to be sponsored, but may succeed if retried. For example, no matching Sponsorship object exists yet, or the Sponsorship requires the sponsor's signature. {% amendment-disclaimer name="Sponsor" /%} |
