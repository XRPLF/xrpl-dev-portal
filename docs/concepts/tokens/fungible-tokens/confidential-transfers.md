...
Confidential transfers are only available for direct payments between accounts. They do not work with other transaction types, such as those involving the XRP Ledger DEX, escrows, or checks.
{% /admonition %}
...
The Confidential Transfers feature is built on three core design principles: the issuer second account model, the multi-ciphertext architecture for privacy and compliance, and the split-balance model for reliable transfers.
...
Because the XRP Ledger treats the second account as a regular _holder_, its balance counts towards tokens in circulation, which allows validators to enforce [supply caps](./multi-purpose-tokens.md#supply-cap) without needing to decrypt confidential balances.

This approach enables confidential distribution, where the issuer converts tokens once on the second account and distributes them directly to users, rather than requiring each holder to individually convert their balances from public to private. Recipients must still [register an encryption key](#key-registration) before they can receive a confidential transfer.
...
Each time your spending balance changes, a **version counter** increments by 1 and is bound to newly generated proofs to prevent [replay attacks](https://en.wikipedia.org/wiki/Replay_attack). Sending confidentially, merging your inbox, converting back to public form, and having your balance clawed back all increment the counter, so any proof generated before one of those changes is no longer valid.
...
The XRP Ledger supports two approaches for auditing hidden balances, on-chain selective disclosure and issuer-mediated auditing. It also provides a specialized clawback mechanism for compliance enforcement.

### On-Chain Selective Disclosure

Issuers can configure an auditor by registering the auditor's ElGamal public key on the MPT issuance with an [MPTokenIssuanceSet transaction][]. This encrypts each holder's confidential balance under the auditor's public key, allowing the auditor to independently decrypt and verify balances off-chain without requiring cooperation from the issuer or holder. For example, a regulator designated as an auditor can use their own private key to decrypt a holder's confidential balance from the ledger, providing independent verification.
...
While issuers retain the same [compliance controls](./multi-purpose-tokens.md#compliance-controls) they have with public transactions, clawback operations work differently for confidential balances.
...
Issuers can enable confidential features by setting the **Can Hold Confidential Balance** flag on an MPT issuance. This can be done either during the initial [MPTokenIssuanceCreate transaction][] or later using an [MPTokenIssuanceSet transaction][].

Like other [MPT issuance flags](../../../references/protocol/ledger-data/ledger-entry-types/mptokenissuance.md#mptokenissuance-flags), enabling the **Can Hold Confidential Balance** flag is a **one-way** operation: no subsequent transaction can disable it.

When enabling confidential transfers, the issuer must also register their ElGamal public key, and optionally an auditor's key. Keys can only be registered with an MPTokenIssuanceSet transaction, so an issuer who sets the flag at creation must follow up with one before holders can convert. No key can be registered once any confidential balance is in circulation.

The MPT issuance must have a transfer fee of **0**, since transfer fees cannot be applied to encrypted amounts. If the issuance has a non-zero transfer fee, the issuer must remove it before enabling confidential transfers.

{% admonition type="warning" name="Caution" %}
If the issuer declares the **Can Hold Confidential Balance** flag [immutable](./mutable-mpts.md#making-mpt-properties-immutable) without enabling it, the MPT issuance can never support confidential transfers.
{% /admonition %}
