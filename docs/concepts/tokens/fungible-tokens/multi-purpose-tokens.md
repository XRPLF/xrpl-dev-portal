...
- **Can Trade:** If this flag is enabled, holders can trade the token on the [Decentralized Exchange (DEX)](../decentralized-exchange/index.md). **Can Transfer** must also be enabled, since non-transferable tokens cannot be traded. Trading MPTs on the DEX requires the [MPTokensV2 amendment](../../../../resources/known-amendments#mptokensv2).
