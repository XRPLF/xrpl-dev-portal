...
**To sell XRP,** you must hold enough XRP to meet all the [reserve requirements](../../accounts/reserves.md), including the reserve for the Offer entry to be placed in the ledger and for the trust line or MPToken entry to hold the token you are buying. As long as you have any XRP left over after setting aside the reserve amount, you can place the offer.
...
- If the sell asset is an MPT and the holder's [MPToken entry is locked](../../../concepts/tokens/fungible-tokens/deep-freeze#how-does-mpt-freezelock-behavior-differ-from-iou).
...
- If the Offer needs to create a new trust line or MPToken entry, but the owner does not have enough XRP for the increased [reserve](../../accounts/reserves.md). (See [Offers and Trust](#offers-and-trust).)
...
To hold a token, you need a ledger entry to track your balance:

- **Trust line tokens** require a [trust line](../fungible-tokens/index.md) to the issuer.
- **MPTs** require an [MPToken entry](../fungible-tokens/multi-purpose-tokens.md).

When an offer is consumed, the ledger automatically creates the necessary trust line or MPToken entry if one doesn't exist. Trust lines created this way have their limit set to 0. Because these count toward your [reserve requirement](../../accounts/reserves.md), any offer that would create a new trust line or MPToken entry also requires enough XRP to cover the additional reserve.
