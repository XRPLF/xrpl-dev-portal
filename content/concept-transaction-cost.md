...
If a sending address has one or more transactions queued, that sender can "push" the existing queued transactions into the open ledger by submitting a new transaction with a high enough transaction cost to pay for all of them. Specifically, the new transaction must increase the average transaction cost of the queued transactions from the same sending address, including itself, so that each of those transactions would be included in the open ledger if the `Fee` value of each of those transactions were equal to the average. The transactions must still follow the other [queuing restrictions](#queuing-restrictions) and the sending address must have enough XRP to pay the transaction costs of all the queued transactions.
