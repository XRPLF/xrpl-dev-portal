...
* `Fee` - Automatically fill in the [Transaction Cost](../../../concepts/transactions/transaction-cost.md) based on the network.

    **Note:** When using `rippled`'s [sign command](../../http-websocket-apis/admin-api-methods/signing-methods/sign.md), you can limit the maximum possible auto-filled value, using the `fee_mult_max` and `fee_div_max` parameters.)
...
The [`Paths` field](types/payment.md#paths) of the [Payment transaction](types/payment.md) type can also be automatically filled in.
...
| Field        | Type   | [Internal Type](../binary-format.md) | Description                      |
|:-------------|:-------|:------------------|:---------------------------------|
