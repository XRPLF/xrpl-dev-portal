...
|:-----------------|:-----------------|:------------------|:-------------------|
| `Destination`    |  String - [Address](../../data-types/basic-data-types.md#addresses) | AccountID    | The address of an account to receive any leftover XRP after deleting the sending account. Must be a funded account in the ledger, and must not be the sending account. |
...
| `tecTOO_SOON` | Occurs if the sender's `Sequence` number is too high. The transaction's `Sequence` number plus 256 must be less than the current [Ledger Index](../../data-types/basic-data-types.md#ledger-index). This prevents replay of old transactions if this account is resurrected after it is deleted. |
