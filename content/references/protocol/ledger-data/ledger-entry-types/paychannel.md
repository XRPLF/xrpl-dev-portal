...
`{% $frontmatter.seo.title %}` entries count as one item towards the owner reserve of the account that created the payment channel, as long as the entry is in the ledger. Removing the channel frees up the reserve; this can only be done after the channel expires (including as a result of being explicitly closed).
...
* The Sequence number of the [PaymentChannelCreate transaction](../../transactions/types/paymentchannelcreate.md) that created the channel
