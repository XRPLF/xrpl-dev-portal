...
For the most part, transactions with `tec` codes take no action other than to destroy the XRP paid as a transaction cost, but there are some exceptions. As one such exception, a transaction that results in `tecOVERSIZE` still cleans up some [unfunded offers](offers.html#lifecycle-of-an-offer). Always look at the [transaction metadata](transaction-metadata.html) to see precisely what a transaction did.

See [transaction cost](transaction-cost.html).
