...
| `temBAD_AMM_OPTIONS`    | The transaction specified invalid options, such as a list of `AuthAccounts` that is too long. ## This is `malformed` now |
| `temDISABLED`           | The AMM feature :not_enabled: is not enabled on this network. |
| `terNO_ACCOUNT`         | One of the accounts specified in the list of `AuthAccounts` does not exist. |
