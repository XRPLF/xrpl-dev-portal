...
| `load_factor_server`             | Number          | _(May be omitted)_ The load factor the server is enforcing, not including the open ledger cost. |
...
| `validated_ledger.reserve_base`  | Number          | The minimum account reserve, as of the most recent validated ledger version. |
| `validated_ledger.reserve_inc`   | Number          | The owner reserve for each item an account owns, as of the most recent validated ledger version. |
...
| `validator_list_expires`         | Number          | _(Admin only)_ When the current validator list will expire, in [seconds since the Ripple Epoch][], or 0 if the server has yet to load a published validator list. |
