...
- You may need to pass different parameters to the `rippled` commandline client to connect to your server. See [Commandline Usage](commandline-usage.html#client-mode-options).
...
The default config file sets the log level to severity "warning" for all categories of log messages by internally using the [log_level method][] during startup. You can control the verbosity of the debug log using the `--silent` commandline option during startup and with the [log_level method][] while the server is running. (See the `[rpc_startup]` stanza of the [config file](commandline-usage.html#verbosity-options) for settings.)
...
The official package installation (for [Ubuntu/Debian](install-rippled-on-ubuntu.html) or [CentOS/RedHat](install-rippled-on-centos-rhel-with-yum.html)) installs such a script by default, to `/opt/ripple/bin/getRippledInfo`. If you compiled `rippled` yourself, you can find the same script [in the `rippled` source code repository](https://github.com/ripple/rippled/blob/develop/bin/getRippledInfo).
