...
Each field of the `cluster` object is the public key of that `rippled` server's identifying keypair. (This is the same value that that server returns as `pubkey_node` in the [`server_info` command](reference-rippled-api-public.html#server-info).) The contents of that field are an object with the following fields:
