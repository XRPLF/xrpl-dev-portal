...
| `server_state`                   | String           | A string indicating to what extent the server is participating in the network. See [Possible Server States](reference-rippled-api-conventions.html#possible-server-states) for more details. |
| `state_accounting`               | Object           | A map of various [server states](reference-rippled-api-conventions.html#possible-server-states) with information about the time the server spends in each. This can be useful for tracking the long-term health of your server's connectivity to the network. [New in: rippled 0.30.1][] |
