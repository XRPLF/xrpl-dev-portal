...
const sendXrpButtonEl = document.getElementById('send-xrp-button')

sendXrpButtonEl.addEventListener('click', () => {
    const address = document.getElementById('input-destination-address').value
    const destinationTag = document.getElementById('input-destination-tag').value
    const amount = document.getElementById('input-xrp-amount').value

    window.electronAPI.onClickSendXrp({address, destinationTag, amount})
})