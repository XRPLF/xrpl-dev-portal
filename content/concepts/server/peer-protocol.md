...
The node key pair also identifies other servers for purposes of clustering or reserving peer slots. If you have a cluster of servers, you should configure each server in the cluster with a unique `[node_seed]` setting. For more information on setting up a cluster, see [Cluster `rippled` Servers](cluster-rippled-servers.html).
...
- If the server is configured as a private peer, it connects _only_ to its fixed peers.
...
- The server asks its direct peers not to reveal its IP address in untrusted communications, including the [peer crawler API response](peer-crawler.html). This does not affect trusted communications such as the `peers` admin method.
...
- **Tutorials:**
    - [Configure Peering](configure-peering.html)