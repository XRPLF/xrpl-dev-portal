...
Anyone can issue tokens by sending a [Payment transaction](../../references/protocol/transactions/types/payment.md) if the necessary trust lines are in place. You can "burn" tokens by sending them back to the issuer. In some cases, cross-currency payments or trades can also create more tokens according to an issuer's settings.
