...
The [server_info method](server_info.html) reports how many ledger versions your server has available in the `complete_ledgers` field.
...
The XRP Ledger identifies data (on several different levels) by a unique hash of its contents. The XRP Ledger's state data contains a short summary of the ledger's history, in the form of the `LedgerHashes` object type. Servers use the `LedgerHashes` object to know which ledger versions to fetch, and to confirm that the ledger data they receive is correct and complete.
...
- **References:**
    - [Ledger Object Types](ledger-object-types.html)