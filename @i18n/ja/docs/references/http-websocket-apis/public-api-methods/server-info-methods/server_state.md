...
| `Field`                          | 型         | 説明            |
|:---------------------------------|:-----------|:-----------------------|
| `amendment_blocked`              | 真偽値      | _（省略される場合があります）_`true`の場合、このサーバは[Amendmentブロック](../../../../concepts/networks-and-servers/amendments.md#amendment-blocked)の状態です。サーバーがAmendmentブロックではない場合、レスポンスではこのフィールドが省略されます。 |
