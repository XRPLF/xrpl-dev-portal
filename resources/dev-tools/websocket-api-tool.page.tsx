...
import { JsonParam, StringParam, useQueryParams, withDefault, QueryParamProvider } from 'use-query-params'
...
export function WebsocketApiTool() {
  ...
  const [selectedConnection, setSelectedConnection] = useState(
    params.server
      ? connections.find((connection) => {
          return connection?.ws_url === params.server
        })
      : connections[0],
  )
  ...
  const [currentBody, setCurrentBody] = useState(JSON.stringify(params.req || currentMethod.body, null, 2))
  ...
  useEffect(() => {
    ...
    newWs.onmessage = function handleMessage(event) {
      ...
      if (data.type === 'response' || !streamPausedRef.current) {
        setResponses((prevResponses) => [JSON.stringify(data, null, 2)].concat(prevResponses))
      }
    }
    ...
    <div className="container-fluid" role="document" id="main_content_wrapper">
      <div className="row">
        <aside className="right-sidebar col-lg-3 order-lg-4" role="complementary">
          <RightSideBar commandList={commandList} currentMethod={currentMethod} setCurrentMethod={setMethod} />
        </aside>
        <main className="main col-lg-9" role="main" id="main_content_body">
          <section className="container-fluid pt-3 p-md-3 websocket-tool" id="wstool-1">
            <h1>{translate('WebSocket Tool')}</h1>
            <div className="api-method-description-wrapper">
              <h3>
                <Link to={currentMethod.link} className="selected_command">
                  {currentMethod.name}
                </Link>
              </h3>
              {currentMethod.description && (
                <p className="blurb">
                  {currentMethod.description}
                  {currentMethod.clio_only ? <ClioOnlyNotice /> : ''}
                </p>
              )}
              {currentMethod.link && (
                <Link className="btn btn-outline-secondary api-readmore" to={currentMethod.link}>
                  {translate('Read more')}
                </Link>
              )}
            </div>
            ...
            <div className="api-input-area pt-4">
              ...
              <div className="request-body">
                <JsonEditor value={currentBody} onChange={handleCurrentBodyChange} />
              </div>
              <div className="btn-toolbar justify-content-between pt-4" role="toolbar">
                <div className="btn-group mr-3" role="group">
                  <button className="btn btn-outline-secondary send-request" onClick={() => sendWebSocketMessage(currentBody)}>
                ...
                <div className="btn-group request-options" role="group">
                  <button
                    className={`btn connection ${connected ? 'btn-success' : 'btn-outline-secondary'} ${connectionError ?? 'btn-danger'}`}
                  ...
                  <PermalinkButton currentBody={currentBody} selectedConnection={selectedConnection} />
                  {!currentMethod.ws_only && <CurlButton currentBody={currentBody} selectedConnection={selectedConnection} />}
                </div>
              </div>
            </div>
            ...
            <div className="api-response-area pt-4">
              ...
              <div className="btn-toolbar justify-content-between response-options" role="toolbar">
