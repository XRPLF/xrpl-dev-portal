...
function FaucetEndpoints({ faucet, givenKey }: { faucet: FaucetInfo; givenKey: string }) {
  ...
  return (
    <div key={givenKey}>
      <h4>
        {faucet.shortName} {translate(`Servers`)}
      </h4>
      <pre>
        <code>
          // WebSocket
...
function FaucetSidebar({ faucets }: { faucets: FaucetInfo[] }): React.JSX.Element {
  return (
    <aside className="right-sidebar col-lg-6 order-lg-4" role="complementary">
      {faucets.map((faucet) => (
        <FaucetEndpoints faucet={faucet} key={faucet.shortName + ' Endpoints'} givenKey={faucet.shortName + ' Endpoints'} />
      ))}
    </aside>
  )
}
        ...
        <main className="main col-md-7 col-lg-6 order-md-3" role="main" id="main_content_body">
          <section className="container-fluid pt-3 p-md-3">
            ...
            <div className="content">
              <p>
                {translate('resources.dev-tool.faucet.content.part1', 'These ')}
                <Link to="../../docs/concepts/networks-and-servers/parallel-networks">
                  {translate('resources.dev-tool.faucet.content.part2', 'parallel XRP Ledger test networks')}
                </Link>{' '}
                {translate(
                  'resources.dev-tool.faucet.content.part3',
                  'provide platforms for testing changes to the XRP Ledger and software built on it, without using real funds.',
                )}
              </p>
              <p>
                {translate('resources.dev-tool.faucet.content.part4', 'These funds are intended for')}{' '}
                <strong>{translate('resources.dev-tool.faucet.content.part5', 'testing')}</strong>{' '}
                {translate(
                  'resources.dev-tool.faucet.content.part6',
                  "only. Test networks' ledger history and balances are reset as necessary. Devnets may be reset without warning.",
                )}
              </p>
              <p>
                {translate(
                  'resources.dev-tool.faucet.content.part7',
              ...
              <p>
                {translate(
                  'resources.dev-tool.faucet.content.part8',
                  'The tool below will generate credentials for you and recharge it immediately; if you want to top up an already existing address, you can do it here:',
                )}{' '}
                <a className="external-link" href="https://test.xrplexplorer.com/faucet" target="_blank">
                  test.xrplexplorer.com/faucet
                </a>
              </p>
              ...
              {faucets.map((net) => (
                <div className="form-check" key={'network-' + net.shortName}>
                  <input
                    onChange={() => setSelectedFaucet(net)}
                    className="form-check-input"
                    type="radio"
                    name="faucet-selector"
                    id={net.id}
                    checked={selectedFaucet.shortName == net.shortName}
                  />
...
async function generateFaucetCredentialsAndUpdateUI(
  ...
  try {
    ...
    alert(
      `${translate('resources.dev-tools.faucet.error.part1', 'There was an error with the ')}${selectedFaucet.shortName}${translate('resources.dev-tools.faucet.error.part2', ' faucet. Please try again.')}`,
    )
  }
