 ...
 */
export function addNewLogEntry(setLogEntries: React.Dispatch<React.SetStateAction<LogEntryItem[]>>, entry: LogEntryItem) {
  setLogEntries((prev) => {
    return [...prev, entry]
  })
}
 ...
 */
export function updateLogEntry(setLogEntries: React.Dispatch<React.SetStateAction<LogEntryItem[]>>, entryToUpdate: LogEntryItem) {
  setLogEntries((prev) => {
...
export function LogEntry({ message, id, status }: LogEntryItem) {
  const { useTranslate } = useThemeHooks()
  const { translate } = useTranslate()
  let icon = undefined
  if (!!status?.icon) {
    icon = (
      <span className={clsx(status.icon?.type === 'SUCCESS' && CLASS_GOOD, status.icon?.type === 'ERROR' && CLASS_BAD)}>
        {status.icon?.label}
