...
export const CurlModal: React.FC<CurlProps> = ({ closeCurlModal, currentBody, selectedConnection }) => {
  ...
  return (
    <Modal id="wstool-1-curl" title={translate('cURL Syntax')} onClose={() => {}} footer={footer}>
      <form>
        <div className="form-group">
          <label htmlFor="curl-box-1">
            {translate(
              'resources.dev-tools.websocket-api.curl.modal.desc.part1',
...
export function CurlButton({ selectedConnection, currentBody }: CurlButtonProps) {
  ...
  return (
    <>
      ...
      {showCurlModal && (
        <CurlModal closeCurlModal={() => setShowCurlModal(false)} currentBody={currentBody} selectedConnection={selectedConnection} />
      )}
    </>
  )
}
