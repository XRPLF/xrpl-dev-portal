...
const PermalinkModal: React.FC<PermaLinkProps> = ({ closePermalinkModal, currentBody, selectedConnection }) => {
  ...
  return (
    <Modal id="wstool-1-permalink" title={translate('Permalink')} footer={footer} onClose={closePermalinkModal}>
      <form>
        <div className="form-group">
          <label htmlFor="permalink-box-1">{translate('Share the following link to load this page with the currently-loaded inputs:')}</label>
          <textarea
            id="permalink-box-1"
...
export function PermalinkButton({ currentBody, selectedConnection }: PermaLinkButtonProps) {
  ...
  return (
    <>
      ...
      {showPermalinkModal && (
        <PermalinkModal closePermalinkModal={() => setShowPermalinkModal(false)} currentBody={currentBody} selectedConnection={selectedConnection} />
      )}
    </>
  )
}
