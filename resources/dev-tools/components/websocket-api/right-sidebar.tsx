...
export const RightSideBar: React.FC<RightSideBarProps> = ({ commandList, currentMethod, setCurrentMethod }) => {
        ...
        {commandList.map((list, index) => (
          <Fragment key={index}>
            ...
            {list.methods.map((method) => (
              <li className={`method${method === currentMethod ? ' active' : ''}`} key={method.name}>
                <Link to={`/resources/dev-tools/websocket-api-tool#${slugify(method.name)}`} onClick={() => setCurrentMethod(method)}>
                  {method.name}&nbsp;
                  {method.status === 'not_enabled' && (
                    <span className="status not_enabled" title="This feature is not currently enabled on the production XRP Ledger.">
                      <i className="fa fa-flask"></i>
                    </span>
                  )}
                  {method.clio_only && <ClioOnlyIcon />}
                </Link>
              </li>
            ))}
          </Fragment>
