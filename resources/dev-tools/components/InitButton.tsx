...
async function onInitClick(props: InitializationProps): Promise<void> {
  const {
    existingClient,
    alert, // From useAlert()
    setClient,
...
export function InitButton({ isInitEnabled, toInit }: { isInitEnabled: boolean; toInit: InitializationProps }): React.JSX.Element {
  ...
  return (
    <div className="form-group">
      <button
        className={clsx('btn btn-primary form-control', isInitEnabled ? '' : 'disabled')}
        type="button"
        id="init_button"
        onClick={() => {
          onInitClick(toInit)
