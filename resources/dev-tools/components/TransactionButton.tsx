  ...
  return (
    <div>
      <div className="form-group" id={id}>
        ...
        {loadingBar?.id && (
          <div className="progress mb-1" id={loadingBar?.id ?? ''}>
            <div
              className={clsx(
                'progress-bar progress-bar-striped w-0',
                loadingBar?.widthPercent < 100 && loadingBar?.widthPercent > 0 && 'progress-bar-animated',
              )}
              style={{
                width: Math.min(loadingBar?.widthPercent + (loadingBar?.defaultOn ? 1 : 0), 100).toString() + '%',
            ...
            {((loadingBar?.widthPercent < 100 && loadingBar?.widthPercent > 0) || (loadingBar.defaultOn && loadingBar?.widthPercent === 0)) && (
              <small className="justify-content-center d-flex position-absolute w-100">{translate(loadingBar?.description)}</small>
            )}
          </div>
        )}
        ...
        <div className="input-group mb-3">
          ...
          <button
            className={clsx(
              'btn btn-primary form-control needs-connection',
        ...
        <small className="form-text text-muted">{content.longerDescription}</small>
      </div>
      <hr />
    </div>
  )
