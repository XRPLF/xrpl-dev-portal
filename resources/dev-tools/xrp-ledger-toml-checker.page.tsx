...
export default function TomlChecker() {
  ...
  const domainButtonProps: TextLookupFormProps = {
    ...
    description: (
      <p>
        {translate('resources.dev-tools.toml-checker.domain.description.part1', `This tool allows you to verify that your `)}
        <code>{translate(`xrp-ledger.toml`)}</code>
        {translate('resources.dev-tools.toml-checker.domain.description.part2', ` file is syntactically correct and deployed properly.`)}
      </p>
    ),
  ...
  const addressButtonProps: TextLookupFormProps = {
    ...
    description: (
      <p>
        {translate(
          'resources.dev-tools.toml-checker.account.description.part1',
          `Enter an XRP Ledger address to see if that account is claimed by the domain it says owns it.`,
        )}
      </p>
    ),
      ...
      <main className="main col-lg-9" role="main" id="main_content_body">
        <section className="container-fluid">
          <div className="p-3">
            ...
            <p>
              {translate(
                'resources.dev-tools.toml-checker.p.part1',
                `If you run an XRP Ledger validator or use the XRP Ledger for your business,
                    you can provide information about your usage of the XRP Ledger to the world in a machine-readable `,
              )}
              <Link to="../../docs/references/xrp-ledger-toml">
                <code>{translate(`xrp-ledger.toml`)}</code>
                {translate('resources.dev-tools.toml-checker.p.part2', ` file`)}
              </Link>
              {translate('resources.dev-tools.toml-checker.p.part3', `.`)}
            </p>
          </div>
