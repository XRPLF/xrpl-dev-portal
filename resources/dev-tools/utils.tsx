...
export function logTx(txName: string, hash: string, finalResult: string, setTxHistory: React.Dispatch<React.SetStateAction<React.JSX.Element[]>>) {
  ...
  const li = (
    <li key={hash} className={clsx('list-group-item fade-in p-1', classes)}>
      {icon} {txName}:{' '}
      <a href={txLink} target="_blank" className="external-link">
        {hash}
      </a>
    </li>
  )
...
): Promise<TxResponse<Transaction> | undefined> {
  ...
  try {
    ...
    if (!silent) {
      if (finalResult === 'tesSUCCESS') {
        ...
        errorNotif(
          submitConstData.alert,
          `${tx.TransactionType} tx failed with code ${finalResult}
                      (hash: ${hash})`,
        )
      }
