    ...
    return advantageContents.map((content) => (
      <div key={content.subtitle}>
        <Link to={content.href}>
          <h5 className="card-subhead">{translate(content.subtitle)}:</h5>
        </Link>
        <div className="card-text">{translate(content?.description || '')}</div>
      </div>
    ))
  } else {
      ...
      <ul className="advantages-list">
        {advantageContents.map((content) => (
          <li key={content.subtitle} className="advantage-item">
            ...
            {content.description && <span className="advantage-description">{translate(content.description)}</span>}
          </li>
        ))}
      </ul>
    )
...
export const AdvantagesSection: React.FC<AdvantagesSectionProps> = ({ title, description, advantages, className = '', useLinks = true }) => {
  const { useTranslate } = useThemeHooks()
  const { translate } = useTranslate()
  ...
  return (
    <section className={`advantages-section container-new py-20 ${className}`}>
      <div className="d-flex flex-column-reverse">
        <p className={headerSpacingClass}>{translate(description)}</p>
        <h4 className="eyebrow mb-8">{translate(title)}</h4>
      </div>
      <div className={`${getGridClass()} nav card-grid`} id="security-features">
