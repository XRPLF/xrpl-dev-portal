...
export const BenefitsSection: React.FC<BenefitsSectionProps> = ({
  ...
  return (
    <section className={`container-new py-26 ${className}`}>
      ...
      {description && <p className="mt-6 mb-0 col-lg-8 col-sm-8 p-0">{translate(description)}</p>}
      <ul className="mt-10 card-grid card-grid-3xN" id={listId}>
        {cards.map((card) => (
          <li className="col ls-none" key={card.id}>
            ...
            <p className="mt-6-until-sm mt-3 mb-0">{typeof card.description === 'string' ? translate(card.description) : card.description}</p>
          </li>
        ))}
      </ul>
    </section>
