...
export const DeveloperResourcesSection: React.FC<DeveloperResourcesSectionProps> = ({ cards, className = '' }) => {
        ...
        {cards.map((card, index) => (
          <div key={index} className={`com-card ${card.backgroundClass || ''}`}>
            <div className="card-content custom-gap">
              ...
              <p className="card-description">{typeof card.description === 'string' ? translate(card.description) : card.description}</p>
              <div className="card-links">
                {card.links.map((link, linkIndex) => (
                  <Link key={linkIndex} className={`com-card-link ${linkIndex === 0 ? 'mt-16' : ''}`} target={link.target || '_blank'} to={link.url}>
                    {translate(link.text)}
                  </Link>
                ))}
              </div>
